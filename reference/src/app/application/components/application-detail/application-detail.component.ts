...
export class ApplicationDetailComponent implements OnInit, OnDestroy {
  ...
  submitReview(): void {
    ...
    if (this.isLPSpecialist) {
      ...
      reviewObservable = this.applicationService.submitSpecialistReview(reviewData);
    } else if (this.isAgricultureManager) {
      console.log('Using submitManagerReview');
      reviewObservable = this.applicationService.submitManagerReview(reviewData);
    } else if (this.isCOO) {
  ...
  canSeeSection(section: string): boolean {
    ...
    if (section === 'documents') {
      ...
      return Array.isArray(this.application.documents) && this.application.documents.length > 0;
    }

    // For reviews section
    if (section === 'reviews') {
      // Only show if reviews exist
      return Array.isArray(this.application.reviews) && this.application.reviews.length > 0;
    }
  ...
  handleActionCompleted(updatedApplication: Application): void {
    ...
    this.updateUserPermissions();
    console.log('Workflow action completed, application updated:', updatedApplication);
