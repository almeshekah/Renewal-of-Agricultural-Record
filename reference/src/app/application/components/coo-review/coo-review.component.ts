...
@Component({
  selector: 'app-coo-review',
  templateUrl: './coo-review.component.html',
  styleUrls: ['./coo-review.component.scss'],
})
export class CooReviewComponent implements OnInit {
  ...
  readonly ROLE = 'COO';
  readonly USER_NAME = 'Test COO'; // Would come from auth service in a real app
  readonly USER_ID = '103'; // Would come from auth service in a real app
      ...
      application => {
        ...
        if (!this.isReviewEligible()) {
          this.errorMessage = this.translateService.instant(
            'application.review.not_eligible_for_coo_review'
          );
        }
      },
      error => {
  ...
  isReviewEligible(): boolean {
    // COO can only review applications that have been approved by the Manager
    return this.application && this.application.status === ApplicationStatus.UNDER_REVIEW_COO;
  }
